...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `ram` WHERE   (`Price`  BETWEEN '$price1' and '$price2') AND CONCAT(`Type`, `Price`) LIKE '%".$valueToSearch."%'" ;
...
<body>
	<div style="color: white; width:100%; margin:auto;background-attachment: fixed; background-image: url('Images/Background1.jpg');">
		...
		<div class="mainbuild">
			...
			<form style="font-size: 2vw; margin: 2%;" action="Ram.php" method="post">
			<ul style="width:100%;">
			...
			<?php while($row = mysqli_fetch_array($search_result)):?>
				...
				<input type="radio" name="radiobutton" value='.$row['RAM_ID'].'/>'.$row['Type'].'</td>
			...
			<?php
				if(isset($_POST['submitpart'])){
					...
					$query="SELECT Product_ID FROM `ram` WHERE '$radioVal'= RAM_ID";}
					...
					if (isset($_POST['radiobutton'])){
						...
						$search_result = myfunction($query);
						 while($row = mysqli_fetch_array($search_result)){
						 	...
						 	$connect = mysqli_connect("localhost", "root", "", "buildit");
						 	$query1= "INSERT INTO `orders`(Customer_ID, Category_Name,Product_ID) VALUES (".$_SESSION['Customer_ID'].",'ram','$pid')";
