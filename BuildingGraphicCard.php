...
if(isset($_POST['search']))
{
    ...
    $query = "Select G.GC_ID,G.GCName,G.Power,G.Price,G.PCIe,G.Memory,G.ClockSpeed FROM Graphic_Card G WHERE (G.Price  BETWEEN '$price1' and '$price2') AND CONCAT(G.GCname, G.Price) LIKE '%".$valueToSearch."%'";
    $result = filterTable($query);
    
}
 else {
    $query = "Select G.GC_ID,G.GCName,G.Power,G.Price,G.PCIe,G.Memory,G.ClockSpeed FROM Graphic_Card G";
    $result = filterTable($query);
}
...
<body>
	...
	<div style="color: white; width:100%; margin:auto;background-attachment: fixed; background-image: url('Images/Background1.jpg');">
		...
		<div class="mainbuild">
			<form style="font-size: 2vw; margin: 2%;" method="POST" action="BuildingGraphicCard.php">
			<ul style="width:100%;">
				...
				<li><?php echo '<input type="hidden" name="pid" value=' . $_POST['pid'] . '>';
						echo '<input type="hidden" name="mbid" value=' . $_POST['mbid'] . '>'; ?> <input type="submit" name="search" value="go"/> </li>
			</ul>
			</form>
			...
			<div class="maintable">
			<form method="POST" action="BuildingRAM.php">
					...
					if (!empty($_POST['pid'])){
						...
						if (mysqli_num_rows($result) > 0){ //Fetch and display results from sql 
							while($row = mysqli_fetch_assoc($result)){
								...
								<input type="radio" name="gcid" value='.$row['GC_ID'].' />'.$row['GCName'].'</td>
