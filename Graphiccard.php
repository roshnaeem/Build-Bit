...
<?php
	...
	//To search Graphiccard according to name and price range
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `graphic_card` WHERE   (`Price`  BETWEEN '$price1' and '$price2') AND CONCAT(`GCName`, `Price`) LIKE '%".$valueToSearch."%'" ;
    $search_result = filterTable($query);
    
}
 else {
 	
    $query = "SELECT * FROM `graphic_card`";
    $search_result = filterTable($query);
}
...
<body>
	<div style="color: white; width:100%; margin:auto;background-attachment: fixed; background-image: url('Images/Background1.jpg');">
		...
		<div class="mainbuild">
			<form style="font-size: 2vw; margin: 2%;" action="Graphiccard.php" method="post">
			<ul style="width:100%;">
				<!-- name -->
				<li class="li-search"> Search <input type="search" name="name" placeholder="Search here"/> </li>
				<!-- price -->
				<li class="li-range"> Price range <input type="number" name="price1" value="0" step="10" min="0"/>
				to <input type="number" name="price2" value="5000" step="10" min="0"/> </li>
				<!-- action -->
			...
			<div class="maintable">
			<form action="Graphiccard.php" method="post">
			<table class="partdisp">
				<?php while($row = mysqli_fetch_array($search_result)):?>
						<?php
                echo '<tr><td class="partcell"> 
				<input type="radio" name="radiobutton" value='.$row['GC_ID'].'/>'.$row['GCName'].'</td>
			...
			<?php
				if(isset($_POST['submitpart'])){
					...
					$query="SELECT Product_ID FROM `graphic_card` WHERE '$radioVal'= GC_ID";}
					if (isset($_POST['radiobutton'])){
						...
						$search_result = myfunction($query);
						 while($row = mysqli_fetch_array($search_result)){
						 	...
						 	$connect = mysqli_connect("localhost", "root", "", "buildit");
						 	$query1= "INSERT INTO `orders`(Customer_ID, Category_Name,Product_ID) VALUES (".$_SESSION['Customer_ID'].",'graphic_card','$pid')";
