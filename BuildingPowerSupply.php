...
if(isset($_POST['search']))
{
	...
	$mbpower = getPower("Motherboard", $_POST['mbid'], "MB_ID");
	$ppower = getPower("processor", $_POST['pid'], "P_ID");
	$gcpower = getPower("graphic_card", $_POST['gcid'], "GC_ID");
	$rpower = getPower("ram", $_POST['rid'], "RAM_ID");
	$mempower = getPower("memory", $_POST['memid'], "Mem_ID");
		
	$totalpowersupply = $mbpower + $ppower +$gcpower +$rpower + $mempower;
						
	$query="SELECT PS.PS_ID,PS.PSName,PS.Power,PS.Price FROM power_supply PS
				WHERE PS.Power >". $totalpowersupply. " AND (PS.Price  BETWEEN '$price1' and '$price2') AND CONCAT(PS.PSName,PS.Power, PS.Price) LIKE '%".$valueToSearch."%'";
    ...
    $query = "SELECT PS.PS_ID,PS.PSName,PS.Power,PS.Price FROM power_supply PS
				WHERE PS.Power >". $totalpowersupply;
    $result = filterTable($query);
}
...
function getPower($TN, $id, $idn){
					$conn = mysqli_connect("localhost", "root", "", "buildit");
					$sql = "Select Power FROM ".$TN." WHERE ".$idn." = ".$id;
...
<body>
	...
	<div style="color: white; width:100%; margin:auto;background-attachment: fixed; background-image: url('Images/Background1.jpg');">
		...
		<div class="mainbuild">
			<form style="font-size: 2vw; margin: 2%;"method="POST" action="BuildingPowerSupply.php">
			<ul style="width:100%;">
				...
				<li> <?php 
						//Storing IDs in hidden input fields to send back to page in case of a search
						...
						echo '<input type="hidden" name="memid" value=' . $_POST['memid'] . '>'; ?><input type="submit" name="search" value="go"/> </li>
			</ul>
			</form>
			...
			<div class="maintable">
			<form method="POST" action="BuildingSummary.php">
			...
			<?php
				...
				if(isset($_POST['submit']) or isset($_POST['search'])){
					if (!empty($_POST['memid'])){
						//Storing IDs in hidden input fields to send to next page
						echo '<input type="hidden" name="rid" value=' . $_POST['rid'] . '>';
						echo '<input type="hidden" name="mbid" value=' . $_POST['mbid'] . '>';
						echo '<input type="hidden" name="pid" value=' . $_POST['pid'] . '>';
						echo '<input type="hidden" name="gcid" value=' . $_POST['gcid'] . '>';
						echo '<input type="hidden" name="memid" value=' . $_POST['memid'] . '>';
						
						if (mysqli_num_rows($result) > 0){
							while($row = mysqli_fetch_assoc($result)){ //Fetching a displaying results
								echo '<tr><td class="partcell"> 
								<input type="radio" name="psid" value='.$row['PS_ID'].' />'.$row['PSName'].'</td>
								<td class="partcell" width="25%">'.$row['Power'].'W'.'</td>
