...
<?php
//To search Motherboard according to name and price range
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `Motherboard` WHERE   (`Price`  BETWEEN '$price1' and '$price2') AND CONCAT(`MBname`, `Price`) LIKE '%".$valueToSearch."%'" ;
...
$MessageFail = "";
				if(isset($_POST['submitpart'])){
					...
					$query="SELECT Product_ID FROM `motherboard` WHERE '$radioVal'= MB_ID";}
					if (isset($_POST['radiobutton'])){
						...
						$search_result = myfunction($query);
						 while($row = mysqli_fetch_array($search_result)){
						 	...
						 	$connect = mysqli_connect("localhost", "root", "", "buildit");
						 	$query1= "INSERT INTO `orders`(Customer_ID, Category_Name,Product_ID) VALUES (".$_SESSION['Customer_ID'].",'motherboard','$pid')";
...
<body>
	...
	<div style="color: white; width:100%; margin:auto;background-attachment: fixed; background-image: url('Images/Background1.jpg');">
		...
		<div class="mainbuild">
			...
			<form style="font-size: 2vw; margin: 2%;" action="Motherboard.php" method="post">
			<ul style="width:100%;">
			...
			<form action="Motherboard.php" method="post" >
			<table class="partdisp">
					<?php while($row = mysqli_fetch_array($search_result)):?>
						<?php
                echo '<tr><td class="partcell"> 
				<input type="radio" name="radiobutton" value='.$row['MB_ID'].' />'.$row['MBname'].'</td>
