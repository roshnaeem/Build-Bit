...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `accessories` WHERE   (`Price`  BETWEEN '$price1' and '$price2') AND CONCAT(`Aname`, `Price`) LIKE '%".$valueToSearch."%'" ;
...
<body>
	...
	<div style="color: white; width:100%; margin:auto;background-attachment: fixed; background-image: url('Images/Background1.jpg');">
		...
		<div class="mainbuild">
			...
			<form style="font-size: 2vw; margin: 2%;" action="Accessories.php" method="post">
			<ul style="width:100%;">
			...
			<table class="partdisp">
				...
				<input type="radio" name="radiobutton" value='.$row['A_ID'].' />'.$row['AName'].'</td>
			...
			<?php
				if(isset($_POST['submitpart'])){
					 ...
					 $query="SELECT Product_ID FROM `accessories` WHERE '$radioVal'= A_ID";}
					if (isset($_POST['radiobutton'])){
						...
						$search_result = myfunction($query);
						 while($row = mysqli_fetch_array($search_result)){
						 	...
						 	$connect = mysqli_connect("localhost", "root", "", "buildit");
						 	$query1= "INSERT INTO `orders`(Customer_ID, Category_Name,Product_ID) VALUES (".$_SESSION['Customer_ID'].",'accessories','$pid')";
