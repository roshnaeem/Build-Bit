...
<?php
	...
	//To search Powersupply according to name and price range
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `power_supply` WHERE   (`Price`  BETWEEN '$price1' and '$price2') AND CONCAT(`PSName`, `Price`) LIKE '%".$valueToSearch."%'" ;
...
<body>
	<div style="color: white; width:100%; margin:auto;background-attachment: fixed; background-image: url('Images/Background1.jpg');">
		...
		<div class="mainbuild">
			...
			<form style="font-size: 2vw; margin: 2%;" action="Powersupply.php" method="post">
			<ul style="width:100%;">
			...
			<div class="maintable">
			<form action="Powersupply.php" method="post" >
			<table class="partdisp">

				<?php while($row = mysqli_fetch_array($search_result)):?>
						<?php
                echo '<tr><td class="partcell"> 
				<input type="radio" name="radiobutton" value='.$row['PS_ID'].'/>'.$row['PSName'].'</td>
			...
			<?php
				if(isset($_POST['submitpart'])){
					 ...
					 $query="SELECT Product_ID FROM `power_supply` WHERE '$radioVal'= PS_ID";}
					if (isset($_POST['radiobutton'])){
						...
						$search_result = myfunction($query);
						 while($row = mysqli_fetch_array($search_result)){
						 	...
						 	$connect = mysqli_connect("localhost", "root", "", "buildit");
						 	$query1= "INSERT INTO `orders`(Customer_ID, Category_Name,Product_ID) VALUES (".$_SESSION['Customer_ID'].",'power_supply','$pid')";
